OctoroitOS.users = {
  ...
  saveSession() {
    OctoroitOS.storage.set('sys:session', {user: this.current, locked: this.isLocked, guest: this.guestMode});
  },
  authenticate(u, p) {
    if (this.guestMode && u === 'guest') {
      this.current = 'guest'; this.isLocked = false; this.saveSession(); return true;
    }
    const user = this.list.find(x => x.username === u);
    if (user && user.pin === p) {
      this.current = u; this.isLocked = false; this.guestMode = false; this.saveSession(); return true;
    }
    return false;
  },
  createUser(u, p, a = '👤') {
  ...
  deleteUser(u) {
    ...
    OctoroitOS.storage.set('sys:users', this.list);
    Object.keys(OctoroitOS.storage.memory).forEach(k => {
      if (k.startsWith(`user:${u}:`)) delete OctoroitOS.storage.memory[k];
    });
  ...
  lock() {
    ...
    this.isLocked = true; this.saveSession(); this.showLockOverlay();
  },
  logout() {
    if (this.guestMode) this.clearGuestData();
    this.current = null; this.isLocked = false; this.guestMode = false; this.saveSession(); location.reload();
  },
  startGuest() {
    this.current = 'guest'; this.guestMode = true; this.isLocked = false; this.saveSession();
    this.hideAuthScreen(); OctoroitOS.bootDesktop();
  },
  clearGuestData() {
    Object.keys(OctoroitOS.storage.memory).forEach(k => {
      if (k.startsWith('user:guest:')) delete OctoroitOS.storage.memory[k];
    });
    OctoroitOS.storage._sync();
  },
  getNamespace() {
    return this.current ? `user:${this.current}` : 'user:guest';
  },
  showLockOverlay() {
